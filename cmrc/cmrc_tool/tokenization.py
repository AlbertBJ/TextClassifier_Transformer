...
class FullTokenizer(object): # what bert uses
...
class BasicTokenizer(object):# tokenize by space
  ...
  def tokenize(self, text):
    ...
    text = self._tokenize_chinese_chars(text) # 也就是在中文字符的前后加上空格，这样后续的分词流程会把每一个字符当成一个词。
  ...
  def _run_split_on_punc(self, text):
    ...
    start_new_word = True
    output = []
    """这个函数会对输入字符串用标点进行切分，返回一个list，list的每一个元素都是一个char。比如输入he’s，则输出是[[h,e], [’],[s]]。"""
    while i < len(chars):
      char = chars[i]
  ...
  def _clean_text(self, text):
    ...
    for char in text:
      cp = ord(char)
      # codepoint == 0 means: nonsense character; codepoint == 0xfffd is shown as � which is used for unrecognized character
...
class WordpieceTokenizer(object): # tokenize by space and then do it tokenize in a more detailede way to WordPiece level
...
def _is_control(char):
  """Checks whether `chars` is a control character. non printable character"""
